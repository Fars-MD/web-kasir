...
async function handler(req: VercelRequest, res: VercelResponse) {
  if (req.method === 'GET') {
    try {
      ...
      const [todayStats, weekStats, monthStats, productCount, recentTx, todayPcs, weekPcs, monthPcs] = await Promise.all([
      ...
      return res.json({
        today: { total: (todayStats.rows[0] as any).total, count: Number((todayStats.rows[0] as any).count), profit: 0, totalPcs: Number((todayPcs.rows[0] as any).total_pcs) },
        week: { total: (weekStats.rows[0] as any).total, count: Number((weekStats.rows[0] as any).count), profit: 0, totalPcs: Number((weekPcs.rows[0] as any).total_pcs) },
        month: { total: (monthStats.rows[0] as any).total, count: Number((monthStats.rows[0] as any).count), profit: 0, totalPcs: Number((monthPcs.rows[0] as any).total_pcs) },
...
export default withSecurity(handler, { methods: ['GET'], rateLimitMax: 20 })
